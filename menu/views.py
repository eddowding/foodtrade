...
def establishment_lookup_name(request):
    ...
    for obj in establishments:
        name = '%s <span class="est_type">%s</span> <span class="est_addr">%s</span>' % (obj.BusinessName, obj.BusinessType, obj.full_address())
...
def menu_render(user):
    ...
    menus = Menu.objects.filter(establishment__in=establishments).order_by('-added_on')
    has_payment = True if Payment.objects.filter(user=user).count() else False
